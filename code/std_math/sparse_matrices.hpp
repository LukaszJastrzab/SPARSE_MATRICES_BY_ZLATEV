...
public:
    ...
    void iterative_refinement(const input_storage_scheme<TYPE>& ISS, TYPE *x, TYPE *b, double acc, size_t max_it, TYPE *d = NULL, TYPE *r = NULL, TYPE *y = NULL)const;
...
//-------------------------------------------------------------------------------------------------
template <typename TYPE>
dynamic_storage_scheme<TYPE>::dynamic_storage_scheme( const input_storage_scheme<TYPE>& ISS,
                                                      double mult1,
                                                      double mult2,
...
//-------------------------------------------------------------------------------------------------
template <typename TYPE>
void dynamic_storage_scheme<TYPE>::LU_decomposition( PIVOTAL_STRATEGY strategy,
                                                     size_t _search,
                                                     double _mult,
                                                     double eps,
                                                     bool pre_sort
                                                     )
{
    if (dynamic_state != ROL_INIT)
    {
        logged_errors += ("dynamic_storage_scheme<TYPE>::LU_decomposition: matrix alredy has been changed - ROL_INIT state is required\n");
        return;
    }
...
*                                if y is not transmitted then function allocates it and deletes it by self
...
*  Method prepares matrix to SOR iterations
*
*  @throw exception     - when stored matrix at least one zero diagonal element
*/
//-------------------------------------------------------------------------------------------------
template <typename TYPE>
void dynamic_storage_scheme<TYPE>::iterative_preparation(void)
{
    if (dynamic_state != ROL_INIT)
    {
        logged_errors += ("dynamic_storage_scheme<TYPE>::iterative_preparation: matrix already has been changed");
        return;
    }
...
int MIN_COLCOST = 0;
    ...
    {
        const double abs_val = fabs(ALU[idx]);
        if (abs_val > max_val)
        {
            max_val = abs_val;
            INDEX = idx;
...
size_t ROW;
    ...
    for (size_t row = stage; row < LastSearch; row++)
    {
        ...
        for (int idx = begin_row; idx <= end_row; idx++)
        {
            ...
            const double temp_abs_val = fabs(ALU[idx]);
    ...
    if (DSS.dynamic_state != COL_INIT)
    {
        for (size_t row = 0; row < DSS.number_of_rows; row++)
        {
            ...
            for (size_t col = 0; col < DSS.number_of_columns; col++)
            {
                ...
                if (!printed && row == col && DSS.PIVOT[row] != 0)
