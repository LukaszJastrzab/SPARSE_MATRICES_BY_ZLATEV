...
void random_vector_values( std::vector< std::complex< T > >* vec, std::complex< T > min_val, std::complex< T > max_val )
{
	...
	for( auto& item : *vec )
	{
		T sign_real = static_cast< T >( bool_rng( g ) ? -1.0 : 1.0 );
		T sign_img = static_cast< T >( bool_rng( g ) ? -1.0 : 1.0 );	

		item = std::complex< T >( sign_real * elem_rng( g ), sign_img * elem_rng( g ) );
	}
}
...
input_storage_scheme< T > generate_ISS( size_t mx_rows, size_t mx_cols, bool non_singularity, int zeros_proportion, T min_val, T max_val )
{
	if( min_val <= 0 || max_val <= 0 || zeros_proportion < 0 )
		throw std::invalid_argument( "Invalid ISS parameters" );

	size_t mx_min_size = std::min( mx_rows, mx_cols );

	input_storage_scheme< T > ISS( mx_rows, mx_cols );
...
template <>
input_storage_scheme< std::complex< float > > generate_ISS( size_t mx_rows,
	size_t mx_cols,
	bool non_singularity,
	int zeros_proportion,
	std::complex< float > min_val,
	std::complex< float > max_val )
{
	...
	input_storage_scheme< std::complex< float > > ISS( mx_rows, mx_cols );
	...
	// ===========
	std::uniform_int_distribution< int > int_rng( 0, zeros_proportion );
	std::uniform_real_distribution< float > elem_rng( min_val.real(), max_val.real() );
	...
	for( size_t row{ 0 }; row < mx_rows; ++row )
		for( size_t col{ 0 }; col < mx_cols; ++col )
				...
				( non_singularity && singular_mx_permuts[ row ] == col ) )
			{
				...
				float sign_img = static_cast< float >( bool_rng( g ) ? -1.0 : 1.0 );

				std::complex< float > val( sign_real * elem_rng( g ), sign_img * elem_rng( g ) );
...
template <>
input_storage_scheme< std::complex< float > > generate_ISS_with_strong_diagonal( size_t mx_rows,
	size_t mx_cols,
	int zeros_proportion,
	std::complex< float > min_val,
	std::complex< float > max_val )
{
	...
	for( size_t col{ 0 }; col < mx_cols; ++col )
	{
		...
		EXPECT_NO_THROW( ISS.add_element( std::complex< float >( sign_real * real_img_val, sign_img * real_img_val ), col, col ) );
	}
